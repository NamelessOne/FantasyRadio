...
public class FileDialog extends ListActivity {
    ...
    private void getDirImpl(final String dirPath) {
        ...
        if (currentPath.equals(PATH_ROOT)) {
            ...
            if (mounted) {
                addItem(mList, PATH_SDCARD + "(SD Card)",
                        this.options.iconSDCard);
        ...
        if (files == null) {
            Toast.makeText(getBaseContext(), "Ошибка открытия SD-карты", Toast.LENGTH_SHORT).show();
            setResult(RESULT_CANCELED, getIntent());
            finish();
            return;
        }
