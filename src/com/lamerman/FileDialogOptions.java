...
class FileDialogOptions {
    ...
    private int iconFile = R.drawable.document;
	...
	private static final String OPTION_CURRENT_PATH_IN_TITLEBAR = "OPTION_CURRENT_PATH_IN_TITLEBAR";
	// Option for one-click select
	private static final String OPTION_ONE_CLICK_SELECT = "OPTION_ONE_CLICK_SELECT";
	// Option for file icon.
	private static final String OPTION_ICON_FILE = "OPTION_ICON_FILE";
	// Option for folder icon.
	private static final String OPTION_ICON_FOLDER = "OPTION_ICON_FOLDER";
	// Option for up icon.
	private static final String OPTION_ICON_UP = "OPTION_ICON_UP";
	...
	public FileDialogOptions(Intent intent) {
		...
		if (intent.hasExtra(SELECTION_MODE)) {
			Log.w("FileDialogOptions",
					"SELECTION_MODE intent value is deprecated. Use FileDialogOptions.allowCreate");
			this.allowCreate = (intent.getIntExtra(SELECTION_MODE,
					SELECTION_MODE_CREATE) == SELECTION_MODE_OPEN);
		} else {
			this.allowCreate = intent.getBooleanExtra(OPTION_ALLOW_CREATE,
					this.allowCreate);
		}

		// Hide the titlebar if needed
		this.titlebarForCurrentPath = intent.getBooleanExtra(
				OPTION_CURRENT_PATH_IN_TITLEBAR, this.titlebarForCurrentPath);

		// One click select
		this.oneClickSelect = intent.getBooleanExtra(OPTION_ONE_CLICK_SELECT,
				this.oneClickSelect);

		// Icons
		this.iconFile = intent.getIntExtra(OPTION_ICON_FILE, this.iconFile);
		this.iconFolder = intent.getIntExtra(OPTION_ICON_FOLDER,
				this.iconFolder);
